...
describe('addSessionKey', () => {
  ...
  it('rejects missing, malformed permissions, and malformed expiration before delegation', () => {
    const client = new AncoreClient({ accountContractId: CONTRACT_ID });

    expect(() => client.addSessionKey(null as unknown as AddSessionKeyParams)).toThrow(
      BuilderValidationError
    );
    expect(() =>
      client.addSessionKey({
        ...params,
        permissions: 'all' as unknown as AddSessionKeyParams['permissions'],
      })
    ).toThrow(BuilderValidationError);
    expect(() =>
      client.addSessionKey({
  ...
  it('rejects missing account contract IDs', () => {
    expect(() => new AncoreClient({ accountContractId: '' })).toThrow(BuilderValidationError);
  });

  it('preserves core SDK errors thrown by dependency writers', () => {
    const expected = new BuilderValidationError('already normalized');
    const writer = {
  ...
  it('wraps generic dependency errors and unknown thrown values', () => {
    expect(() =>
      addSessionKey(
        {
          addSessionKey: jest.fn(() => {
